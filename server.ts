...
async function startServer() {
  ...
  // Cache YouTube results to prevent rate limiting and improve speed
  ...
  // API Route to fetch latest YouTube videos dynamically
  app.get("/api/youtube/latest", async (req, res) => {
    const apiKey = process.env.YOUTUBE_API_KEY;
    const channelUrl = "https://www.youtube.com/@Nexora-k9x";

    // Curated fallback list of 5 premium long videos matching Somil's channel content
    const defaultVideos = [
      {
        id: "92NGRk_LF-4",
          ...
          if (channelId) {
            ...
            if (apiRes.ok) {
              ...
              if (apiData.items && apiData.items.length > 0) {
                const liveVideos = apiData.items.map((item: any, i: number) => {
                  ...
                  const category = title.toLowerCase().includes("pcb") || title.toLowerCase().includes("circuit")
                    ? "Hardware PCBs"
                    : title.toLowerCase().includes("cad") || title.toLowerCase().includes("solidworks")
                      ? "Product Design"
  ...
  // API Route to fetch YouTube channel statistics dynamically
  app.get("/api/youtube/stats", async (req, res) => {
    ...
    try {
      ...
      if (response.ok) {
          ...
          try {
            ...
            if (apiRes.ok) {
              ...
              if (apiData.items && apiData.items.length > 0) {
                ...
                if (subCountInt >= 1000000) {
                  subscriberCount = (subCountInt / 1000000).toFixed(1) + "M";
                } else if (subCountInt >= 1000) {
                  subscriberCount = (subCountInt / 1000).toFixed(1) + "K";
                } else {
        ...
        // Regex Scraper for Subscriber Count
        const subMatch = html.match(/"subscriberCountText"\s*:\s*\{\s*"accessibility"\s*:\s*\{\s*"accessibilityData"\s*:\s*\{\s*"label"\s*:\s*"([^"]+)"/i) ||
                         ...
                         html.match(/itemprop="interactionCount" content="([^"]+)"/i);
        ...
        if (subMatch) {
          ...
          if (subClean.toLowerCase().includes("k")) val *= 1000;
          if (subClean.toLowerCase().includes("m")) val *= 1000000;
          ...
          if (broadSubMatch) {
            ...
            if (subscriberCount.toLowerCase().includes("k")) val *= 1000;
            if (subscriberCount.toLowerCase().includes("m")) val *= 1000000;
            subscriberValue = isNaN(val) ? 12420 : val;
          }
        }

        // Regex Scraper for Video Count
        const vidMatch = html.match(/"videoCountText"\s*:\s*\{\s*"runs"\s*:\s*\[\s*\{\s*"text"\s*:\s*"([^"]+)"/i) ||
                         html.match(/"videoCountText"\s*:\s*\{\s*"simpleText"\s*:\s*"([^"]+)"/i);
        if (vidMatch) {
  ...
  // Serve Vite in dev mode, or compiled build in production mode
